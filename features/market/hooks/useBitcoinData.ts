...
export const useBitcoinData = () => {
    ...
    const { data, isLoading: queryLoading, refetch } = useHistoricBitcoinDataQuery();
    ...
    const replacePointsAtomically = bitcoinStore(state => state.replacePointsAtomically);
    const applyBufferedPoints = bitcoinStore(state => state.applyBufferedPoints);
