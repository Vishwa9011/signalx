...
const RoundControlForm = ({
  isRoundId = true,
  round: selectedRound,
  onResolve,
}: RoundControlFormProps) => {
  ...
  const [formDetails, setFormDetails] = React.useState<FormDetails>(() =>
    deriveFormState(selectedRound, activeTab),
  );
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    if (isPriceInFloat) {
      return alert(
        "Please enter a valid integer price without decimal places.",
      );
    }
